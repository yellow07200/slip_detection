...
//SlipDetectionRawTest::SlipDetectionRawTest():  left_grippers("baxter_left_gripper_action/gripper_action","left", false),total_time_(0.), total_events_(0), total_corners_(0)

SlipDetectionRawTest::SlipDetectionRawTest():  total_time_(0.), total_events_(0), total_corners_(0)
{



// Subscribers
    davis_sub_ = pnh_.subscribe("/dvs/events", 0, &SlipDetectionRawTest::Davis_Slip_detection_Callback, this);
    ftbiased_sub_ = pnh_.subscribe("/transformed_world", 0, &SlipDetectionRawTest::FTBiasedCallback, this);
    gripper_ = pnh_.subscribe("/robot/end_effector/left_gripper/state", 0, &SlipDetectionRawTest::gripperCallback, this);
...
//ROS service
	goal_service = pnh_.advertiseService("grasp_command", &SlipDetectionRawTest::ServiceCallback, this);
	sample_service = pnh_.advertiseService("sample_command", &SlipDetectionRawTest::ServiceCallbacksample, this);
	exp_reset_service = pnh_.advertiseService("exp_reset_service", &SlipDetectionRawTest::ServiceCallbackexperiment, this);
...
threshold_edge=0;
threshold_corner= 0;threshold_flat= 0; only_slip_raw= 0; threshold_raw=0;
...
void SlipDetectionRawTest::FTBiasedCallback(const geometry_msgs::WrenchStamped::ConstPtr &biased_ft_msg)
{
...
void SlipDetectionRawTest::gripperCallback(const baxter_core_msgs::EndEffectorState::ConstPtr &gripper_msg)
{
	Gripper_Percent=*gripper_msg;
}

void SlipDetectionRawTest::Davis_Slip_detection_Callback(const dvs_msgs::EventArray::ConstPtr &msg)
{
// Packets definition
...
//Separating Noise,edge,corner from Harris7 detector
	double th1= 0.1;//-0.000001;//-0.0000001,
	double th2= 8;//3;//7 //8
...
//  	  	  	if (count_corner > corner_raw_max) {corner_raw_max= count_corner;}
			...
			threshold_raw= (raw_max+ (raw_max*0.1));
...
//					slip_start_control=false;
			slip_raw_frame_events =only_slip_raw ;
...
SlipDetectionRawTest::~SlipDetectionRawTest()
{
...
void SlipDetectionRawTest::run_controller_gripper()
{
...
bool SlipDetectionRawTest::ServiceCallbacksample(object_test::Request  &req,object_test::Response &res)
{
...
bool SlipDetectionRawTest::ServiceCallback(object_test::Request  &req,object_test::Response &res)
{
if (req.event_capture_command ==0)
{
...
bool SlipDetectionRawTest::ServiceCallbackexperiment(object_test::Request  &req,object_test::Response &res)
{
...
//uint64_t SlipDetectionRawTest::timeSinceEpochMillisec() {
////	std::chrono:
////
...
void SlipDetectionRawTest::slip_detection_frames(){
	ros::Rate rate(100);
...
//		std::cout <<"flat ----" <<events_features[2]<< std::endl;
		...
		std::cout <<"new raw ----" <<slip_raw_frame_events<< std::endl;
		std::cout <<"oldraw ----" <<slip_edge_frame<< std::endl;
...
void SlipDetectionRawTest::publish_frame_millisec(){
...
//}
//	FT_readings_interval=FT_Biased;
...
void SlipDetectionRawTest::publish_complete_msg(){
//	//	 collecting complete datset
//		Data.header=head_data;
//	//	Data.condition= condition;
//	//	Data.object = object;
...
//		Data.slip_duration_SS= slip_duration_SS;
//
//		Data.ft_raw_data = FT_readings_interval;
//		Data.f_max_data = f_max_data;
//		Data.t_max_data = t_max_data;
//		Data.gripper_percent= Gripper_Percent.position;
//		Data.Exp_count= experiment_count;
