...
soft_hand_slip_detection::soft_hand_slip_detection(): left_gripper("baxter_left_gripper_action/gripper_action","left", false) ,total_time_(0.), total_events_(0), total_corners_(0)
{
...
//    gripper_command_pub = pnh_.advertise<baxter_core_msgs::EndEffectorCommand>("/robot/end_effector/left_gripper/command",10);
    ...
    complete_data =pnh_.advertise<slip_detection_davis::SlipDetectionDataComplete_analysis_final4>("/slip_complete_data", 1);
...
//	          command_gripper.sender = "slip_detection_node";
  ...
  s_events_features[0]=0;  s_events_features[1]=0; s_events_features[2]=0;
...
sample_count_raw= 0; sample_count_flat= 0; sample_count_corner= 0; sample_count_edge= 0; only_slip_raw= 0; one_time=true;

control_edge_raw_max= 2; control_corner_raw_max= 2;force_temp=0;
e_max=0;
c_max=0;

grasp_edge_raw_max=0;
grasp_corner_raw_max=0;
}
...
void soft_hand_slip_detection::FTBiasedCallback(const geometry_msgs::WrenchStamped::ConstPtr &biased_ft_msg)
{
	...
	F_squared_grasp = sqrt(pow(FT_Biased.wrench.force.x,2)+ pow(FT_Biased.wrench.force.y,2)+ pow(FT_Biased.wrench.force.z,2));
					T_squared_grasp = sqrt(pow(FT_Biased.wrench.torque.x,2)+ pow(FT_Biased.wrench.torque.y,2)+ pow(FT_Biased.wrench.torque.z,2));
...
{
//    	 if ( arc_detector.isCornerARC(e);
    	  double isharris= isCorner_Edge_HARRISi (e);   // Harris Detector
//double isharris= 1;   // Harris Detector

//    	  bool fast= isCornerFAST(e);
//    	  bool arc= isCornerARC(e);
...
//    	  if ( isCornerFAST(e)) // Fast corner
//        	  if ( isCornerARC(e)) // corner
        	 // if ( isharris)
...
//			 		grasp_count_corner_positive =count_corner_positive; grasp_count_corner_negative =count_corner_negative;
	      	  	  	if (count_edge > grasp_edge_raw_max) {grasp_edge_raw_max= count_edge;}
	      	  	  	if (count_corner > grasp_corner_raw_max) {grasp_corner_raw_max= count_corner;}
...
//		 grasp_duration_SS = elaspedTime_grasp;
		 ...
		 slip_monitor=true;
		 grasp_stop =false;
		 std::cout <<"Grasp edge sample noise----" <<grasp_edge_raw_max<< std::endl;

		      	  	  	       	    	  std::cout <<"Grasp corner sample noise----" <<grasp_corner_raw_max<< std::endl;
		}
...
//				if (((count_raw >= (raw_max+ (raw_max*0.1)))  && !slip_timer))
							...
							threshold_flat= (flat_raw_max+ (flat_raw_max*0.1));
							threshold_edge= (edge_raw_max+ (edge_raw_max*0.1));
							threshold_corner= (corner_raw_max+ (corner_raw_max*0.1));

					if ((count_edge >= threshold_edge) && (count_corner >= threshold_corner) && !slip_timer)

		{
//						count_edge_incremental=count_edge;
//						count_corner_incremental=count_corner;
...
//								 								 slip_corner_frame = only_slip_corner + threshold_corner;
			 ...
			 slip_raw_frame_events =count_edge+count_corner;
...
//						slip_total_events_flat +=count_flat;
				...
				if ((count_edge <= edge_raw_max)  && (count_corner <= threshold_corner) )
				{
...
//      	  	  	if (count_edge > count_edge_incremental) {count_edge_incremental= count_edge;}
//      	  	  	if (count_corner > count_corner_incremental) {count_corner_incremental= count_corner;}
//
//				if ((count_edge >count_edge_incremental)  && (count_corner > count_corner_incremental) )
//				{
//					slip_start_control=true;
//					slip_end =true;
////					slip_start= false;
...
//	    left_gripper.goToPosition(39.5);
//	    left_gripper.command_
//	    left_gripper.closeGripper();
//	    left_gripper.goToPosition(35);

//	    ros::Duration(4.0).sleep();
//
//	    left_gripper.openGripper();
//	    ros::Duration(4.0).sleep();
//	  	left_gripper.closeGripper_slipcontol_command(1);
...
if (req.event_capture_command ==1)
{
	...
	//control_start=true; //change: add this sentence
	//slip_seperate_monitor=true;
...
grasp_open=true;
	  ...
	  one_time=true;control_start=false;
	  count_edge_incremental=0;
	 				  count_corner_incremental=0;
	 				 events_features[0]=0;
									 events_features[0]=0;
													 s_events_features[0]=0;
																	 s_events_features[0]=0;
//slip_start_control =false;
	  	  //  left_gripper.openGripper();
	  	   // ros::Duration(4.0).sleep();
...
//		events_features[2]=only_slip_flat;
			 ...
			 // if ((events_features[0] >= control_edge_raw_max ) && (events_features[1] >= control_corner_raw_max))
    	  	  //	{
			  edge_f=events_features[0];
			  corner_f=events_features[1];

//    	  	  	if (events_features[0] > count_edge_incremental) {count_edge_incremental= events_features[0];}
//    	  	  	if (events_features[1] > count_corner_incremental) {count_corner_incremental= events_features[1];}

				if ((events_features[0] >count_edge_incremental)  && (events_features[1] > count_corner_incremental) )
				{
//					slip_start_control=true;
//					slip_end =true;
////					slip_start= false;
					e_max=grasp_edge_raw_max*2;
					c_max=grasp_corner_raw_max*2;
//e_max=100; //45 200  100 80
//c_max=13;  //6  15  15  12

			  if (events_features[0]>e_max) events_features[0]=e_max;
			  if (events_features[1]>c_max) events_features[1]=c_max;
			  s_events_features[0] =(events_features[0]-1)/(e_max-1)*370+50;
			  s_events_features[1] =(events_features[1]-1)/(c_max-1)*35;

//			  	  if ((s_events_features[0] <35 || s_events_features[0]>420) || (s_events_features[1]<1 || s_events_features[1]>35 ))
//				  {
//					  cmd_grip_force=0;
//				  }
//				  else
//				  {
					  fuzzy2inputs_initialize();
					   std::cout << "Scaled_edge=" <<s_events_features[0]<<" edge="<<events_features[0]<< std::endl;
					   std::cout << "Scaled Corner=" <<s_events_features[1]<<" corner="<<events_features[1]<< std::endl;
					  cmd_grip_force = fuzzy2inputs(s_events_features);
//				  }


				  if (cmd_grip_force>=force_temp)
				  {
						std::cout <<"cmd_grip_force----" <<cmd_grip_force<< std::endl;

					  	left_gripper.closeGripper_slipcontol_command(cmd_grip_force);
					  force_temp=cmd_grip_force;

//					  std::cout << "Command Scaled_edge=" <<s_events_features[0]<<" Command edge_500microSec="<<events_features[0]<< std::endl;
//					  std::cout << "Command Scaled Corner=" <<s_events_features[1]<<" Command corner_500microSec="<<events_features[1]<< std::endl;

				  }
				  else
				  {
					  cmd_grip_force=force_temp;
				  }


//	    rate.sleep();
				  count_edge_incremental= events_features[0];
				  count_corner_incremental= events_features[1];
				}
//			      	  	  	}
	    //control_start=false; //change
...
void soft_hand_slip_detection::slip_control_optic(){
//	ros::Rate rate(1);
//
//	while (ros::ok())
//		{
...
//	control_start=false;
		...
		left_gripper.closeGripper_slipcontol_command(cmd_grip_force);
		cmd_grip_force=cmd_grip_force+1;
...
//		Data.slip_count_flat_negative=slip_count_flat_negative;
		...
		if (slip_seperate_monitor)//  && one_time)
		{

			events_features[0]=slip_edge_frame;
			events_features[1]=slip_corner_frame;
//			std::cout << "1stedge=" <<events_features[0]<< std::endl;
//								   std::cout << "1stcorner=" <<events_features[1]<< std::endl;
	...
	Data.slip_total_events_edge= slip_edge_frame;
	Data.slip_total_events_corner= slip_corner_frame;
	Data.slip_total_events_noise= slip_flat_frame;
	...
	Data.gripper_percent= cmd_grip_force;
...
//	Only_slip_data.slip_total_events_noise= slip_total_events_flat;
	...
	Data.Baxter_Gripper_Force_Commanded= cmd_grip_force;
	Data.Baxter_Gripper_Force_Measured= Gripper_Percent.force;
	Data.Force= F_squared_grasp;
	Data.Torque= T_squared_grasp;
	Data.Slip_Detection_= slip_start*200;
	Data.Slip_Detection= slip_raw_frame_events;
